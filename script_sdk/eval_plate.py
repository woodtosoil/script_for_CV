...
def rec_from_txt(filename):
    ...
    for gt in gts:
        ...
        # print(gt)
        obj_struct = {}
        obj_struct['name'] = gt[0]
        # print(obj_struct['name'])
        obj_struct['bbox'] = [int(gt[1]),
                              int(gt[2]),
                              int(gt[3]),
                              int(gt[4])]
...
def voc_eval(detpath,
    ...
    if not os.path.isfile(cachefile):
        ...
        for i, imagename in enumerate(imagenames):
            # print(imagename)
            imagename = imagename.replace(".xml","")
            # print(imagename)
    ...
    for file in detfile:
        ...
        for line in lines:
            ...
            # print(confidence)
                BB.append([float(z) for z in line[1:5]])
            else:
                continue
            # plate.append(line[5:])
...
if __name__ == '__main__':
    det_path = "D:\\datasets\\Face++\\hanhui_result\\"
    anno_path = "D:\\datasets\\Face++\\plate_xml\\plate_xml\\"
    cache_path = './'
    #格式要求：txt：首位为类别，接下来为四个坐标，最后放置信度
    rec,prec,ap = voc_eval(detpath=det_path,annopath=anno_path,classname="car",cachedir=cache_path,ovthresh=0.5)
