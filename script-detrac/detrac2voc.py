             ...
             if(target.tag=="target"):
                 ...
                 for target_child in target:
                     ...
                     if(target_child.tag=="attribute"):
                         ...
                         trajectory_length=doc.createElement('trajectory_length')
                         truncation_ratio=doc.createElement('truncation_ratio')

                         name_value=target_child.attrib["vehicle_type"]
                         orientation_value=target_child.attrib['orientation']
                         speed_value=target_child.attrib['speed']
                         trajectory_length_value=target_child.attrib['trajectory_length']
                         truncation_ratio_value=target_child.attrib["truncation_ratio"]

                         name.appendChild(doc.createTextNode(str(name_value)))
                         orientation.appendChild(doc.createTextNode(str(orientation_value)))  #随意指定
                         speed.appendChild(doc.createTextNode(str(speed_value)))  #随意指定
                         trajectory_length.appendChild(doc.createTextNode(str(trajectory_length_value)))  #随意指定
                         truncation_ratio.appendChild(doc.createTextNode(str(truncation_ratio_value)))
 
                         
                         object.appendChild(name)
                         object.appendChild(orientation)
                         object.appendChild(speed)
                         object.appendChild(trajectory_length)
...
if ( __name__ == "__main__"):
   #print("main")
   basePath="/media/root/硬盘2/detrac/DETRAC-Train-Annotations-XML"
